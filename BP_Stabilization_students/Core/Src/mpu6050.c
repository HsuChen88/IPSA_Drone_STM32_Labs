...
void mpu6050_read_raw_data(imu_raw_data_t* imu_raw){
	...
	/*mpu6050_read(MPU6050_ACCEL_XOUT_H_REG, 6, data);
	imu_raw->accel_x = (data[0]<<8) | data[1];
	imu_raw->accel_y = (data[2]<<8) | data[3];
	imu_raw->accel_z = (data[4]<<8) | data[5];

	mpu6050_read(MPU6050_GYRO_XOUT_H_REG, 6, data);
	imu_raw->gyro_x = (data[0]<<8) | data[1];
	imu_raw->gyro_y = (data[2]<<8) | data[3];
	imu_raw->gyro_z = (data[4]<<8) | data[5];*/
}
